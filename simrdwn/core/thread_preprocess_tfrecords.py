...
class Prep_TFrecordThread(QtCore.QThread):
    ...
    ###############################################################################
    def load_pbtxt(self, pbtxt_filename, verbose=False):
        """
        ...
        verbose : boolean
            Switch to self.sig1.emit relevant values to screen.  Defaults to ``True``.
        ...
        for i, line_raw in enumerate(f.readlines()):
            ...
            if (prev_line.startswith('id:')) and (line.startswith('name:')):
                ...
                if verbose:
                    self.sig1.emit("line: "+ line)
                    self.sig1.emit(" prev_line: "+ prev_line)
                    self.sig1.emit(" label: "+ label)
            prev_line = line

        if verbose:
            self.sig1.emit("label_dict: "+ str(label_dict))
        f.close()
    ...
    ###############################################################################
    # from convert.py
    def convert_reverse(self, size, box):
    ...
    ###############################################################################
    # from convert.py
    def convert_bbox_yolt_to_tf(self, height, width, yolt_row):
        ...
        [x0, x1, y0, y1] = self.convert_reverse((width, height), box)
    ...
    ###############################################################################
    def yolt_to_tf_example(self, image_file, label_file,
                        label_map_dict,
                        convert_dict={},
                        # cat_int_plus=1,
        ...
        if verbose:
            self.sig1.emit("image_file: "+ image_file)
        ...
        if verbose:
            self.sig1.emit("image shape: "+ str(im.shape))
        ...
        if len(label_file) > 0:
            ...
            for idx, row in df.iterrows():
                ...
                #box = [x_frac, y_frac, width_frac, height_frac]
                # get pixel coords
                [x0, x1, y0, y1] = self.convert_bbox_yolt_to_tf(height, width, row)
        ...
        if verbose:
            self.sig1.emit("  len objects:"+ str(len(xmin)))
            self.sig1.emit("  classes: "+ str(classes))
            self.sig1.emit("  classes_text: "+str(classes_text))
            #self.sig1.emit("  type classes_text:", type(classes_text))
    ...
    ###############################################################################
    def test_image_tf_example(self, image_file, verbose=False):
        """
        ...
        if verbose:
            self.sig1.emit("image shape:"+ im.shape)
    ...
    ###############################################################################
    def yolt_dir_to_tf(self, image_dir, label_map_dict, TF_RecordPath,  # cat_int_plus=1,
                    convert_dict={},
                    verbose=False):
        ...
        for image_file in image_files:
            ...
            tf_example = self.yolt_to_tf_example(image_file_full, label_file_full,
                                            label_map_dict,
    ...
    ###############################################################################
    def yolt_imlist_to_tf(self, image_list_file, label_map_dict, TF_RecordPath,
        ...
        for i, image_file in enumerate(lines):  # floc.readlines():
            ...
            if verbose:
                self.sig1.emit("image_file:"+ image_file_full)
                self.sig1.emit("  os.path.exists(image_file)="+str(os.path.exists(image_file_full)))
            label_file_full = image_file_full.split('.')[0].replace('images', 'labels_yolo') + '.txt'
        ...
        self.sig1.emit("len records: "+ str(len(lines)))
        return


    ###############################################################################
    def testlist_to_tf(self, image_list_file,  TF_RecordPath, verbose=False):
        ...
        for i, image_file in enumerate(lines):  # floc.readlines():
            image_file_full = image_file.strip()
            if verbose:
                self.sig1.emit("image_file_full:"+ image_file_full)
                self.sig1.emit("  os.path.exists(image_file_full)="+str(os.path.exists(image_file_full)))
            tf_example = self.test_image_tf_example(image_file_full, verbose=verbose)
            if (tf_example):
                writer.write(tf_example.SerializeToString())
        ...
        self.sig1.emit("Time to run testlist_to_tf(): "+ str(time.time() - t0)+ " seconds")
