...
from Format import VOC, COCO, UDACITY, KITTI, YOLO, CustomFormat
...
python example.py \
    --datasets VOC \
    --img_path /media/tunguyen/Devs/DeepLearning/aircraft_detection/simrdwn/data/COWC/images/12TVL120100-CROP_split \
    --label /media/tunguyen/Devs/DeepLearning/aircraft_detection/simrdwn/data/COWC/Annotations_split/12TVL120100-CROP_split \
    --convert_output_path /media/tunguyen/Devs/DeepLearning/aircraft_detection/simrdwn/data/COWC/labels/12TVL120100-CROP_split \
    --img_type ".png" \
    --manipast_path /media/tunguyen/Devs/DeepLearning/aircraft_detection/simrdwn/data/COWC/manifast/12TVL120100-CROP_split \
    --cls_list_file /media/tunguyen/Devs/DeepLearning/aircraft_detection/simrdwn/data/COWC/cowc.names
'''
...
def main(config):
    ...
    if config["datasets"] == "custom":
        custom = CustomFormat()
        yolo = YOLO(os.path.abspath(config["cls_list"]))

        flag, data = custom.parse(config["label"])

        if flag == True:

            flag, data = yolo.generate(data, cls_id=0)
            if flag == True:
            ...
            print("CustomFormat : {}, msg : {}".format(flag, data))

    elif config["datasets"] == "VOC":
        voc = VOC()
        yolo = YOLO(os.path.abspath(config["cls_list"]))
