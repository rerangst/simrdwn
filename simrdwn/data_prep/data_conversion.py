...
class DataConversionThread(QtCore.QThread):
    ...
    def printProgressBar(self, iteration, total, prefix='', suffix='', decimals=1, length=100, fill='█'):
        ...
        self.sig1.emit("\r%s|%s| %s%% (%s/%s)  %s" %
                       (prefix, bar, percent, iteration, total, suffix))
...
class CustomFormat:
    def __init__(self, data_conversion_thread):
        self.data_conversion_thread = data_conversion_thread
        return
    ...
    def parse(self, path):
        try:
            ...
            progress_cnt = 0
            self.data_conversion_thread.printProgressBar(0, progress_length, prefix='\nCustomFormat Parsing:'.ljust(
            ...
            for filename in filenames:
                ...
                while obj_index < num_obj:
                    ...
                    xmin, ymin, xmax, ymax = content[obj_index+1].split(' ')
                ...
                data[filename.split(".txt")[0]] = annotation

                self.data_conversion_thread.printProgressBar(progress_cnt + 1, progress_length,
                                                             prefix='CustomFormat Parsing:'.ljust(15), suffix='Complete', length=40)
                progress_cnt += 1
...
class YOLO:
    ...
    def __init__(self, cls_list_path, data_conversion_thread):
    ...
    def parse(self, label_path, img_path, img_type=".png"):
        try:
            ...
            for filename in filenames:
                ...
                self.data_conversion_thread.printProgressBar(progress_cnt + 1, progress_length, prefix='YOLO Parsing:'.ljust(15), suffix='Complete',
                                                             length=40)
                progress_cnt += 1
    ...
    def generate(self, data, cls_id=None):
        ...
        try:
            ...
            for key in data:
                ...
                for idx in range(0, int(data[key]["objects"]["num_obj"])):
                    ...
                    if cls_id is None:
                        cls_id = self.cls_list.index(
                            data[key]["objects"][str(idx)]["name"])
                ...
                self.data_conversion_thread.printProgressBar(progress_cnt + 1, progress_length, prefix='YOLO Generating:'.ljust(15),
                                                             suffix='Complete',
                                                             length=40)
                progress_cnt += 1
    ...
    def save(self, data, save_path, img_path, img_type):
        ...
        try:
            ...
            for key in data:
                ...
                outfile = os.path.abspath(os.path.join(
                    save_path, "".join([key, ".txt"])))
                with open(outfile, "w") as output_txt_file:
                    output_txt_file.write(data[key])

                self.data_conversion_thread.printProgressBar(progress_cnt + 1, progress_length, prefix='YOLO Saving:'.ljust(15),
                                                             suffix='Complete',
                                                             length=40)
                progress_cnt += 1
